...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--test",
            action="store_true",
            help="Test connection to found radios"
        )
        parser.add_argument(
            "--update-env",
            action="store_true",
            help="Update .env file with found radio port"
        )

    def handle(self, *args, **options):
        if not SERIAL_AVAILABLE:
            self.stdout.write(self.style.ERROR(
                "pyserial not installed. Install with: uv add pyserial"
            ))
            return
    ...
    def test_connection(self, port):
        ...
        if not MESHCORE_AVAILABLE:
            self.stdout.write(self.style.WARNING(
                "   meshcore library not available, skipping connection test"
            ))
            return
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(
                f"   ✓ Port opened successfully at {settings.MESHCORE_BAUD_RATE} baud"
            ))
            ...
            try:
                ...
                try:
                    ...
                    self.stdout.write(self.style.SUCCESS(
                        "   ✓ Successfully read stats from radio"
                    ))
